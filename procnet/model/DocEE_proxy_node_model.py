...
from procnet.model.modules.softmax_moe_router import TargetConditionedMoERouter
...
class DocEEProxyNodeModel(DocEEBasicModel):
    def __init__(self,
        ...
        self.use_tgid_router = bool(getattr(config, 'use_tgid_router', False) or getattr(config, 'use_softmax_moe', False))
        self.tgid_gate = getattr(config, 'tgid_gate', 'softmax')
        if getattr(config, 'use_softmax_moe', False):
            self.tgid_gate = 'softmax'
        self.tgid_variant = getattr(config, 'tgid_variant', 'full')
        self.tgid_tc_weight = float(getattr(config, 'tgid_tc_weight', 0.01))
        self.tgid_tc_temperature = float(getattr(config, 'tgid_tc_temperature', 0.5))
        self.tgid_curriculum = bool(getattr(config, 'tgid_curriculum', True))
        self.tgid_warmup_epochs = int(getattr(config, 'tgid_warmup_epochs', 5))
        self.current_epoch = 0
        if self.tgid_variant in {'core', 'no_tc'}:
            self.tgid_tc_weight = 0.0
        if self.tgid_variant == 'core':
            self.tgid_curriculum = False
        if self.use_tgid_router:
            self.tgid_router = TargetConditionedMoERouter(
                hidden_size=self.node_size,
                num_event_types=self.num_event_type,
                num_event_roles=self.num_event_relation,
                num_experts=getattr(config, 'num_experts', 4),
                dropout=self.dropout_ratio,
                gate=self.tgid_gate,
                activation=getattr(config, 'tgid_activation', 'threshold'),
                threshold=getattr(config, 'tgid_threshold', None),
                learnable_threshold=getattr(config, 'tgid_learnable_threshold', True),
                top_k=getattr(config, 'route_top_k', 2),
                use_role_condition=self.tgid_variant != 'type_only',
                use_schema_condition=self.tgid_variant != 'no_init',
                event_role_mask=self.build_event_role_mask(preparer),
            )
    ...
    def should_use_tgid_router(self):
        if not bool(getattr(self, 'use_tgid_router', False)):
            return False
        if not self.training:
            return True
        if not bool(getattr(self, 'tgid_curriculum', True)):
            return True
        return int(getattr(self, 'current_epoch', 0)) > int(getattr(self, 'tgid_warmup_epochs', 5))
    ...
    def forward(self,
        ...
        route_records = []
        tgid_tc_loss = torch.FloatTensor([0]).to(self.device)
        route_logits = None
        use_tgid_router = self.should_use_tgid_router()
        if use_tgid_router:
            proxy, route_prob, route_logits = self.apply_tgid_router(proxy)
            tgid_tc_weight = float(getattr(self, 'tgid_tc_weight', 0.0))
            if self.training and tgid_tc_weight > 0:
                tgid_tc_loss = self.compute_tgid_target_contrastive_loss(route_prob, mention_to_index, events_labels)
            if getattr(self.config, 'dump_route_prob', False) or not self.training:
                route_records = self.build_route_records(
                    doc_id=example.doc_id,
                    mention_to_index=mention_to_index,
        ...
        if route_records:
            records['route_records'] = route_records

        loss = torch.FloatTensor([0]).to(self.device)
        if self.training == True:
            if len(events_labels) == 0:
                ...
                null_event_role_losses = self.ce_none_reduction_loss_fn(
                    proxy_span_relation_logit.reshape(-1, proxy_span_relation_logit.shape[2]),
                    null_event_role_label,
                ).view(num_all_proxy_slot, self.num_event_relation)
                null_event_role_losses = torch.mean(null_event_role_losses, dim=-1)

                loss += torch.sum(null_event_type_losses + null_event_role_losses)
                if self.gradient_accumulation_steps is not None:
                    loss = loss / self.gradient_accumulation_steps
            ...
            for proxy_index, event_index in proxy_to_index.items():
                ...
                not_ignore_num = torch.sum(not_ignore_mask, dim=-1).clamp_min(1)
                event_horizontal_role_losses = torch.sum(event_horizontal_role_losses, dim=-1) / not_ignore_num
                event_horizontal_role_losses = torch.nan_to_num(event_horizontal_role_losses, nan=0.0, posinf=1e4, neginf=-1e4)

                event_loss_matrix = event_type_losses + event_horizontal_role_losses
                event_loss_matrix = torch.nan_to_num(event_loss_matrix, nan=1e4, posinf=1e4, neginf=-1e4)
                if pad_num > 0 :
            ...
            if len(not_match_event) != 0:
                ...
                other_proxy_index = torch.arange(0, other_proxy.shape[0], device=other_proxy.device)[other_proxy]
                
                proxy_num = other_proxy_index.shape[0]
                event_num = not_match_event.shape[0]
                if proxy_num == 0:
                    null_loss += torch.sum(null_loss_matrix)
                    not_match_event = []
                    other_proxy[:] = False
                    used_proxy[:] = 1
                else:
            ...
            loss += mean_event_loss + 0.3 * loss_bio + 0.01 * role_pri_loss + 0.005 * contrastive_loss
            tgid_tc_weight = float(getattr(self, 'tgid_tc_weight', 0.0))
            if use_tgid_router and tgid_tc_weight > 0:
                loss += tgid_tc_weight * tgid_tc_loss
            """
    ...
    def build_event_role_mask(self, preparer):
        event_role_mask = torch.zeros((self.num_event_type, self.num_event_relation), dtype=torch.float)
        for event_type_id, role_ids in preparer.event_schema_index.items():
            for role_id in role_ids:
                event_role_mask[event_type_id, role_id] = 1.0
        return event_role_mask

    def apply_tgid_router(self, proxy):
        # Route argument/proxy representations with a target-conditioned MoE,
        # then collapse routed type views back to the original EPAL shape
        # expected by the current decoder.
        routed_proxy, route_prob, route_logits = self.tgid_router(proxy.unsqueeze(0))
        if self.num_event_type > 1:
            proxy = torch.mean(routed_proxy[0, :, 1:, :], dim=1)
        else:
            proxy = torch.mean(routed_proxy[0], dim=1)
        return proxy, route_prob[0], route_logits[0]

    def build_route_records(self, doc_id, mention_to_index, events_labels, route_prob, route_logits=None):
        mention_gold_types = {key: set() for key in mention_to_index}
        mention_gold_links = {key: [] for key in mention_to_index}
        for event_idx, event_label in enumerate(events_labels):
            event_type_id = int(event_label['EventType'])
            for key, role_ids in event_label.items():
                if key == 'EventType' or key not in mention_to_index:
                    continue
                mention_gold_types[key].add(event_type_id)
                for role_id in role_ids:
                    mention_gold_links[key].append((event_idx, event_type_id, int(role_id)))

        records = []
        for key, mention_index in mention_to_index.items():
            gold_type_ids = sorted(mention_gold_types.get(key, set()))
            links = mention_gold_links.get(key, [])
            roles = {role_id for _, _, role_id in links}
            type_instances = {}
            for event_idx, event_type_id, _ in links:
                type_instances.setdefault(event_type_id, set()).add(event_idx)
            is_cross_type = len(gold_type_ids) >= 2
            is_same_type_multi_instance = any(len(instances) > 1 for instances in type_instances.values())
            is_cross_role = len(roles) > 1
            is_m2m_all = len(links) > 1
            if is_cross_type:
                category = 'CROSS_TYPE_M2M'
            elif is_same_type_multi_instance:
                category = 'SAME_TYPE_MULTI'
            elif is_cross_role:
                category = 'CROSS_ROLE_M2M'
            else:
                category = 'SINGLE'
            arg_key = tuple(int(x) for x in key) if isinstance(key, tuple) else key
            records.append({
                'doc_id': doc_id,
                'arg_key': arg_key,
                'arg_text': str(arg_key),
                'gold_event_type_ids': gold_type_ids,
                'category': category,
                'is_m2m_all': is_m2m_all,
                'is_cross_type': is_cross_type,
                'is_same_type_multi_instance': is_same_type_multi_instance,
                'is_cross_role': is_cross_role,
                'route_prob': route_prob[mention_index].detach().cpu(),
                'route_logits': route_logits[mention_index].detach().cpu() if route_logits is not None else None,
                'activated_experts': torch.nonzero(route_prob[mention_index].detach().cpu() > 0, as_tuple=False).tolist(),
            })
        return records

    def compute_tgid_target_contrastive_loss(self, route_prob, mention_to_index, events_labels):
        mention_gold_types = {key: set() for key in mention_to_index}
        for event_label in events_labels:
            event_type_id = int(event_label['EventType'])
            for key in event_label:
                if key != 'EventType' and key in mention_to_index:
                    mention_gold_types[key].add(event_type_id)

        losses = []
        for key, type_ids in mention_gold_types.items():
            if not type_ids:
                continue
            mention_index = mention_to_index[key]
            valid_gold = [type_id for type_id in type_ids if 0 <= type_id < route_prob.shape[1]]
            if not valid_gold:
                continue
            gold_routes = route_prob[mention_index, valid_gold]
            gold_routes = F.normalize(gold_routes, p=2, dim=-1)
            if gold_routes.shape[0] >= 2:
                sim = torch.matmul(gold_routes, gold_routes.transpose(0, 1))
                eye = torch.eye(gold_routes.shape[0], device=route_prob.device, dtype=torch.bool)
                losses.append(torch.exp(sim[~eye] / self.tgid_tc_temperature).mean())

            all_type_ids = torch.arange(route_prob.shape[1], device=route_prob.device)
            gold_mask = torch.zeros(route_prob.shape[1], device=route_prob.device, dtype=torch.bool)
            gold_mask[torch.tensor(valid_gold, device=route_prob.device)] = True
            neg_type_ids = all_type_ids[~gold_mask]
            if neg_type_ids.numel() > 0:
                neg_routes = F.normalize(route_prob[mention_index, neg_type_ids], p=2, dim=-1)
                neg_sim = torch.matmul(gold_routes, neg_routes.transpose(0, 1))
                losses.append(torch.exp(neg_sim / self.tgid_tc_temperature).mean())

        if not losses:
            return torch.FloatTensor([0]).to(route_prob.device)
    ...
    def pre_event(self, proxy, span_rep, cls_embedding, node_span_to_index):
        ...
        decode_event_type_logit = event_type_logit
        decode_relation_logit = proxy_span_relation_logit
        if not self.training:
            event_type_threshold = float(getattr(self.config, 'event_type_threshold', 0.0))
            role_threshold = float(getattr(self.config, 'role_threshold', 0.0))
            if event_type_threshold > 0:
                decode_event_type_logit = event_type_logit.detach().clone()
                event_type_prob = F.softmax(event_type_logit, dim=-1)
                non_null_prob = event_type_prob[:, 1:]
                keep_non_null = non_null_prob.max(dim=-1).values >= event_type_threshold
                decode_event_type_logit[~keep_non_null, self.null_event_type_index] = event_type_logit.max().detach() + 1.0
            if role_threshold > 0:
                decode_relation_logit = proxy_span_relation_logit.detach().clone()
                role_prob = F.softmax(proxy_span_relation_logit, dim=-1)
                none_index = role_prob.shape[-1] - 1
                non_null_prob = role_prob[:, :, :none_index]
                keep_non_null = non_null_prob.max(dim=-1).values >= role_threshold
                decode_relation_logit[~keep_non_null, none_index] = proxy_span_relation_logit.max().detach() + 1.0
        event_type_prob = decode_event_type_logit.detach().cpu().numpy().tolist()
        event_relation_prob = decode_relation_logit.detach().cpu().numpy().tolist()

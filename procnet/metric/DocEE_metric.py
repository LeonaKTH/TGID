...
class DocEEMetric(BasicMetric):
    ...
    @staticmethod
    def _event_m2m_bucket(event_ans):
        arg_targets = {}
        for event_index, event in enumerate(event_ans):
            event_type = event.get('EventType')
            for key, role_ids in event.items():
                if key == 'EventType':
                    continue
                if key not in arg_targets:
                    arg_targets[key] = []
                values = role_ids if isinstance(role_ids, list) else [role_ids]
                for role_id in values:
                    arg_targets[key].append((event_type, role_id, event_index))
        ...
        for targets in arg_targets.values():
            ...
            type_instances = {}
            for event_type, _, event_index in targets:
                type_instances.setdefault(event_type, set()).add(event_index)
            if len(event_types) > 1:
                has_cross_type = True
            if any(len(instances) > 1 for instances in type_instances.values()):
    ...
    def _score_event_subset(self, event_ans_all, event_pred_all, selected_indices):
        ...
        subset_ans = [event_ans_all[index] for index in selected_indices]
        subset_pred = [event_pred_all[index] for index in selected_indices]
        _, score_results = self.dee_score_fn(subset_ans, subset_pred)
        score_results['doc_num'] = len(selected_indices)
    ...
    def the_score_fn(self, results: List[dict]):
        ...
        if 'event_ans' in results[0] and 'event_pred' in results[0]:
            ...
            for i in range(len(event_ans_all)):
                ...
                bucket = self._event_m2m_bucket(ea)
                for name in m2m_subset_indices:
                    if bucket[name]:
                        m2m_subset_indices[name].append(i)
            ...
            m2m_score_results = {
                name: self._score_event_subset(event_ans_all, event_pred_all, indices)
                for name, indices in m2m_subset_indices.items()
            }
    ...
    def dee_score_fn(self, events_ans: List[List[dict]], events_pred: List[List[dict]]):
        ...
        if len(events_ans) >= 1000:
            artifact_dir = Path(getattr(self, 'artifact_dir', '.'))
            artifact_dir.mkdir(parents=True, exist_ok=True)
            torch.save(
                {'pred': pred_record_mat_list, 'gold': gold_record_mat_list, 'copy_num': copy_num},
                artifact_dir / 'all.pt',
            )
